...
namespace GeneralTool.General.SocketLib.Packages
{
    ...
    public class CustomCommandPackage : IPackage<ReceiveState>
    {
        ...
        // (set) Token: 0x0600060A RID: 1546 RVA: 0x0001E19B File Offset: 0x0001C39B
        ...
        public void Subpackage(ReceiveState state, Action<IEnumerable<byte>, Socket> completePackage)
        {
            ...
            if (!flag)
            {
                bool flag2 = this.buffer == null || this.buffer.Length == 0;
                if (flag2)
                {
                ...
                else
                {
                    ...
                    for (; ; )
                    {
                        int index = list.IndexOf(this.buffer[0], PreIndex);
                        bool flag4 = index == -1;
                        if (flag4)
                        {
                            break;
                        }
                        int endIndex = this.GetEndIndex(list, this.buffer, index);
                        ...
                        {
                            int count = endIndex;
                            IEnumerable<byte> tmp = list.Skip(PreCount).Take(count - PreCount - this.buffer.Length);
                            PreCount = count;
                            PreIndex = count;
        ...
        public byte[] Package(byte[] bytes)
        {
            ...
            byte[] newBuffer = new byte[bytes.Length + this.buffer.Length];
            bytes.CopyTo(newBuffer, 0);
            Array.Copy(this.buffer, 0, newBuffer, len, this.buffer.Length);
            ILog log2 = this.Log;
            log2?.Debug("压包完成");
            return newBuffer;
        }
