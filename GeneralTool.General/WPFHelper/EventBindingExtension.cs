...
namespace GeneralTool.General.WPFHelper
{
    ...
    public class EventBindingExtension : MarkupExtension
    {
        ...
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            var str = System.Windows.Markup.XamlWriter.Save(target.TargetObject);
        ...
        Delegate GetHandler(object dataContext, EventInfo eventInfo, string eventHandlerName)
        {
        ...
        Type[] GetParameterTypes(Type delegateType)
        {
            var invokeMethod = delegateType.GetMethod("Invoke");
            return invokeMethod.GetParameters().Select(p => p.ParameterType).ToArray();
        }

        object GetDataContext(DependencyObject target)
        {
            return target.GetValue(FrameworkElement.DataContextProperty)
                ?? target.GetValue(FrameworkContentElement.DataContextProperty);
        }

        readonly Dictionary<Type, Delegate> _dummyHandlers = new Dictionary<Type, Delegate>();

        Delegate GetDummyHandler(Type eventHandlerType)
        {
        ...
        Delegate CreateDummyHandler(Type eventHandlerType)
        {
