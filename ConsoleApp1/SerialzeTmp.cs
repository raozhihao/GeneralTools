...
{
    public class SerializeHelpers2
    {
        #region Public 方法
        ...
        {
            return (T)Desrialize(data,typeof(T));
        }
        ...
        /// <returns>
        /// </returns>
        public object Desrialize(byte[] data, Type type)
        {
            //Type type = null;
            ...
            //    string typeStr = typeData.ToString();//获取当前类型信息
        ...
        private object DesrializeArrary(byte[] data, Type type)
        {
            ...
            using (MemoryStream stream = new MemoryStream(data))
            {
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    object elObj = Desrialize(cdata, elType);
                    //将数据加入进去
        ...
        private object DesrializeClass(byte[] data, Type type)
        {
            ...
            using (MemoryStream stream = new MemoryStream(data))
            {
                ...
                { 
                    ...
                    //获取属性名称
                    var nameLen = binary.ReadInt32();
                    if (nameLen == 0)
                        continue;
                    var nameBytes = binary.ReadBytes(nameLen);
                    var name = Encoding.UTF8.GetString(nameBytes);
                    var p1 = properties.FirstOrDefault(p => p.Name == name);
                    if (p1 != null)
                    {
                        ...
                        object proObj = Desrialize(cdata,p1.PropertyType);

                        p1.SetMethod?.Invoke(obj, new object[] { proObj });
                    }
                ...
                using (MemoryStream stream = new MemoryStream(data))
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        if (argTypes.Length == 1)
                        {
                            ...
                            object proObj = Desrialize(cdata, argTypes[0]);
        ...
        private byte[] SerializeClass(object param, Type type)
        {
            ...
            foreach (PropertyInfo item in properties)//循环类的每一个属性
            {
                ...
                //与入属性的名称
                data.AddRange(this.GetParamerNameBytes(item.Name));
                //写入当前属性的长度
        ...
        private List<byte> GetParamerNameBytes(string name)
        {
            var list = new List<byte>();
            var nameBytes = Encoding.UTF8.GetBytes(name);
            list.AddRange(BitConverter.GetBytes(nameBytes.Length));
